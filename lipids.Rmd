...
z1 <- matrix(NA, nsims, length(xp))
z2 <- matrix(NA, nsims, length(xp))
z3 <- matrix(NA, nsims, length(xg))
z4 <- matrix(NA, nsims, length(xg))

library(coda)

#tiff(filename="zetas.tif", res=300, unit="cm", height=17, width=17)
...
for(i in 1:nsims) z1[i,] <- zs$z[i,1] + zs$z[i,2]*xp
...
for(i in 1:nsims) z2[i,] <- zs$z[i,4] + zs$z[i,5]*xp
...
for(i in 1:nsims) z3[i,] <- zs$z[i,1] + zs$z[i,3]*xg
...
for(i in 1:nsims) z4[i,] <- zs$z[i,4] + zs$z[i,6]*xg
